<script>
  ...
  const statuses = ['operating', 'proposed', 'construction', 'cancelled', 'retired', 'shelved'];
  ...
  const sampleClassesParam = JSON.stringify([
    {
      id: 'coal-operating',
  ...
  const rangeHistogram = [12, 25, 45, 78, 92, 85, 67, 43, 28, 15, 8, 4, 2, 1];
  ...
  const componentIndex = [
    ...
    { name: 'Citation', path: 'src/lib/components/Citation.svelte', category: 'badges' },
    {
      ...
      path: 'src/lib/components/EntityMicroCard.svelte',
      category: 'cards',
    },
    ...
    {
      name: 'LoadingWrapper',
      path: 'src/lib/components/LoadingWrapper.svelte',
      category: 'states',
    },
    {
      name: 'ScreenerLayout',
      path: 'src/lib/components/ScreenerLayout.svelte',
      category: 'layouts',
    },
  ];
