<script>
  ...
  // Track key sequences (e.g., g + key)
  let waitingForSecondKey = false;

  // Commands - initialized in onMount after callbacks are defined
  /** @type {Array<{id: string, label: string, shortcut?: string, action: () => void, section: string}>} */
  let commands = [];
  ...
  onMount(() => {
    // Initialize commands with callbacks
