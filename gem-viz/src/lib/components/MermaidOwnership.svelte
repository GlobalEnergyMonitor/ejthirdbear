<script>
   ...
   *
   * Generates Mermaid flowchart syntax from ownership edges
  ...
  // Generate Mermaid syntax from edges
  function generateMermaidSyntax() {
    if (!edges.length) return '';

    // Helper to strip parentheses and special chars
    ...
    // Build node lookup including asset
    const fullNodeMap = new Map(nodeMap);
    if (assetId && assetName) {
      fullNodeMap.set(assetId, { Name: assetName });
    }
    ...
    const lines = uniqueEdges.map((e, i) => {
      ...
      // Handle special node types that should be unique per edge
...
<div class="mermaid-ownership" bind:this={container}>
  {#if loading}
    <div class="loading">
  ...
  {:else}
    <div class="empty">
      <p>No ownership data to display</p>
    </div>
  {/if}
</div>
