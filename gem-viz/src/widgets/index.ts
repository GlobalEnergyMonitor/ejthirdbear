...
// ============================================================================
// SELF-HEALING — fix stalled mounts from cached embed.js
// ============================================================================

/**
 * On some host pages (Drupal/Webflow), the cached embed.js promise chain
 * stalls for dynamically-injected embeds. This module is always loaded fresh
 * (cache-busted via version.json). When imported, scan for any .gem-embed
 * elements that were marked initialized but never got a shadow root, and
 * mount them directly.
 */
if (typeof document !== 'undefined') {
  setTimeout(() => {
    const stalled = document.querySelectorAll(
      '.gem-embed[data-gem-initialized][data-src]'
    );
    stalled.forEach((el) => {
      if (el.shadowRoot || !(el instanceof HTMLElement)) return;
      const dataSrc = el.getAttribute('data-src');
      if (!dataSrc) return;

      // This element was initialized but never mounted — fix it
      const parsed = parseSrc(dataSrc);
      if (!parsed.type || !WIDGET_MAP[parsed.type]) return;

      const shadow = el.attachShadow({ mode: 'open' });
      el.innerHTML = ''; // clear loading skeleton
      injectStyles(shadow);
      const target = document.createElement('div');
      target.className = 'gem-widget-root';
      shadow.appendChild(target);

      const loader = WIDGET_MAP[parsed.type];
      loader().then((mod) => {
        const instance = mount(mod.default as ReturnType<typeof mount>, { target, props: parsed.props });
        mountedWidgets.set(shadow, instance);
      });
    });
  }, 1000);
}
